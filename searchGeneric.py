...
class Searcher(Displayable):
    ...
    def search(self, time_limit=None):
        count=0
        """returns (next) path from the problem's start node to a goal node. If                                      time_limit is not none, it should be a approximate number of seconds to run before                          timing out.  Returns None if no path exists or the search times out.                                     """
        if time_limit:
            start = time.time()
            skip = 0
        ...
        while not self.empty_frontier():
            ...
            self.num_expanded += 1
            if (count< self.frontier.__len__()):
              count=self.frontier.__len__()
