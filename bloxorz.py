...
class Board(object):
    ...
    def read_board(file):
        ...
        # >>> fake_CAFile = open("CABoard.txt", "r")
        # >>> board0 = Board.read_board(fake_CAFile)
        # >>> board0.goal
        # (4,7)
...
def next_position(pos, action, reverse_direction=False):
    ...
    coords=[item for x in t_list for item in x]
    #print(coords) #gives a list
#in case of up:
#x1,x2 are same and y1 and y2 are same
...
#the y corrdtinates are same : 1 and 3 index
      ...
      #cases for vertical 
      #the x corrdinates are same: 0 and 2 index same
    elif coords[1]!=coords[3] and coords[0]==coords[2] :
      if action =="U":
        coords[1]-=1
        coords[3]-=2
      elif action == "D":
        ...
        #converting a list to lists of lists
    new_list = [coords[i:i+2] for i in range(0, len(coords), 2)]
      #converting list to tuple 
    position = [tuple(x) for x in new_list]
    #converting tuples back to tuples and returns it
